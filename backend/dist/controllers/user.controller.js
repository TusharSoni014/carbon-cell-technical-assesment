...
const login = async (req, res) => {
    ...
    try {
        ...
        if (userId.includes("@")) {
            existingUser = await user_model_1.User.findOne({ email: userId });
        }
        else {
            existingUser = await user_model_1.User.findOne({ username: userId });
        }
        if (!existingUser) {
        ...
        return res.status(200).send({
            ...
            email: existingUser.email,
            token: jwtToken,
            message: "Your token is set in your browser cookies.",
        });
    }
    catch (error) {
